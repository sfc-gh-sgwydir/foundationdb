...
void genprefix(char* str, char* prefix, int prefixlen, int prefixpadding, int rows, int len) {
...
int cleanup(FDBTransaction* transaction, mako_args_t* args) {
	...
	genprefix(prefixstr, KEYPREFIX, KEYPREFIXLEN, args->prefixpadding, args->rows, args->key_length + 1);
	int len = sprintf(beginstr, "%s%c", prefixstr, 0x00);
	sprintf(endstr, "%s%c", prefixstr, 0xff);
